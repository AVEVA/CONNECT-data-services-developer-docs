...
In the cases where you have an asset which references an existing asset type (via the AssetTypeId property), the measurement mappings on the asset is ignored if the asset type has a corresponding type reference. In these cases, the measurement mapping must be configured on the asset type.
