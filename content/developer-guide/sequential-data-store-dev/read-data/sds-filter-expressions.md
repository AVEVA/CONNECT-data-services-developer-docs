...
Filter expressions can be applied to read operations such as `List Values` (<xref:sds-stream-data#list-values-account-path> or <xref:sds-stream-data#list-values-tenants-path>) and `List Summaries` (<xref:sds-stream-data#list-summaries-account-path> or <xref:sds-stream-data#list-summaries-tenants-path>) that return Sequential Data Store (SDS) values.
