...
1. Copy the ID for the community containing streams you want to access.

    From the **URI** field, set the URI to the  [List Communities a Tenant is Joined to](../api-reference/identity/communities.md#list-communities-a-tenant-is-joined-to) endpoint, and then query for the list of communities that your tenant is connected to. Copy the `Id` of a community that you want to access data from.
...
1. From the **Body** tab, copy the relevant stream's `Self` link and paste it into the **URI** field.
