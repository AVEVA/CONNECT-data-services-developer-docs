...
# Run the PI to Data Agent Configuration Utility

Use the PI to Data Agent Configuration Utility to set up and configure AF server and Data Archive data sources before creating a data transfer. After installing or upgrading a PI to Data Agent, use the utility to select a source AF server or Data Archive, view connection details, create AF and PI mappings, set data privacy settings, and register the agent. 
...
| 3  | Displays the PI to Data Agent's status.                        |
| 4  | Provides details about the agent's registration state.        |
| 5  | Currently running PI to Data Agent version.                    |
...
## Open the PI to Data Agent Configuration Utility

The PI to Data Agent Configuration Utility opens after you install or upgrade a PI to Data Agent. You can open the PI to Data Agent Configuration Utility at any time to change server connections and other settings after initial setup. 
...
To open the PI to Data Agent Configuration Utility, follow these steps:

1. Select **Start** > **OSIsoft** > **PI to Data Agent Configuration Utility**, and then select **Yes** to confirm.

   Upon successful authentication, the PI to Data Agent Configuration Utility opens.
...
1. In the `PI to Data Agent Configuration Utility` window, select the **AF** button.<!--Angela Flores 11/12/21 - I normally would not use button, but I can't tell from the screenshots what you actual need to select.-->
...
**Note:** There is a one-to-one (1:1) Data Archive to PI to Data Agent constraint for PI to Data transfers. If your AF server references multiple Data Archives, only one Data Archive can be selected and configured for the transfer.  

The list of available PI Data Archive servers is based on the servers referenced by AF elements on the AF server you selected. If you are upgrading an agent, the PI to Data Agent Configuration Utility maintains the previously selected PI Data Archive configuration.  

**Note:** If you are not adding an AF server, select the PI Data Archive icon on the first screen of the PI to Data Agent Configuration Utility.
...
1. To open the `PI to Data Agent Settings` window, select the pencil icon to the right of **Agent Service Account**.

1. To publish the hostname, select the **Opt-in to publishing PI to Data Agent Hostname in Data Hub?** option under `Data Privacy`.

1. (Optional) In the **PI to Data Agent Description** field, enter a descriptive name for the agent.

1. To save your selections, select **Ok** , and then select **Save** in the utility.

## List of agent status states

It may take a few minutes for a PI System to register with AVEVA Data Hub. The table below lists the various states that may appear under the **Agent Status** field in the PI to Data Configuration Utility.

| **State**                     | **Description**                                              |
| ----------------------------- | ------------------------------------------------------------ |
| Data Source Connection Issue | Indicates the PI To Data Agent cannot connect to the PI Data Archive. Some reasons for this status include the PI Data Archive is turned off, a firewall issue is preventing connections, or an incorrect name is configured for the Data Archive. For example, the agent is trying to connect to a machine that does not exist or was renamed. There may be additional reasons for this status. |
...
| Missing Configuration         | The PI Data Archive server has not been configured in the PI to Data Agent. |
| Registration Failed           | Contact [AVEVA Customer Support](https://softwaresupport.aveva.com) for assistance.             |
| Registering                   | The PI to Data Cloud portion is creating the necessary resources for your PI to Data Agent. |
| Shutdown                      | The last communication that the PI to Data Cloud had with the agent was a shutdown message. |
