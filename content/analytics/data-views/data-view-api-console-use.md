...
| continuationToken | The field for specifying a continuation token when a request returns. For more information on this field and the **Load from Response** ![load from response](../../_icons/default/code-json.svg) button, see [Continuation token](#continuation-token). |
...
When the **Load from Response** ![load from response](../../_icons/default/code-json.svg) button is enabled, the continuation token from the last API console data view request is loaded into the `continuationToken` field automatically. For more information, see <xref:apiConsole#continuation-token>
