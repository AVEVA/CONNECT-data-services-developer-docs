...
| 4       | Not present                                           | `Id`="md_id"<br/> `Name`="**m5**"<br/>  `Value`=30           | `Id`="md_id"<br/> `Name`="**m5**"<br/>  `Value`=30        | m5 appears on the default shape with value =30, along with `UOM` and `SdsTypeCode` from the asset. Because these properties do not exist on the asset type for this `Metadata` value, the default shape takes the values of the asset. |
