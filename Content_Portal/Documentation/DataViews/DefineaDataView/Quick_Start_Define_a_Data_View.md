...
| StreamReferenceNames *(Coming Soon)* | String[] | Optional | [ ] | Identifies the stream reference when referencing an asset property. `StreamReferenceNames` only applies to source `FieldSource.PropertyId`. If more than one key is specified, they are matched as exclusive-or. A key has to be a non-null value.
