...
1. In the `Select Stream` window, click the stream that will be the basis for creating the asset rule. 
...
1. In the **1. Match** list, pick the choice that describes how the value is identified in the stream name. <!-- WRITER'S NOTE: I will add an explanation of each of the options after this gets revised. -->
...
1. Select the checkboxes for each stream metadata that you want to use to construct your assets. 

1. Click **Back to Stream Name Pattern**.
...
1. Click the ![Select token icon](images/select-token-icon.png) icon to open the `Select Stream Reference Name Token` window.

1. Select the token that identifies the part of the stream name that references the measurement on the asset. Click **Select**. <!--WRITER'S NOTE: This step is still not very clear. Please review it again after the UI changes which may make it clearer. -->
...
1. Select each token in the `Token Mappings Status` list.

1. In the `Token Value Mappings` pane on the right, choose either of the following: <!-- WRITER'S NOTE: Changes are coming. -->
...
1. To map a different value in place of the token values, click **Rename Token Values**. Then click **Generate Mappings** to display the list of token values. 

1. In the `Generate Mappings` window, click **Generate** to confirm that you want to overwrite mappings with server-generated mappings. <!--WRITER'S NOTE: This message is changing. -->
...
1. For the measurement token, select **Rename Token Values**, and then click **Generate Mappings** to generate the list of token values. 

1. Click in the text box to display a list of measurements or properties that are defined in the asset type. Select the correct value to map to the token value. 
...
1. To add additional mappings, click **Add Mapping**. 

1. When you have completed configuring the token values for all your tokens, indicated by the green check mark, click **Next**.
...
1. Select the token to add it to the text box. 
